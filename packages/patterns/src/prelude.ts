/**
 * Generates the sandbox prelude JS that runs INSIDE the QuickJS sandbox.
 *
 * Defines the pattern API (ctx) and bootstrap functions. The prelude
 * uses only `var`/`function` at top level so globals persist across
 * evalCode calls. Grid dimensions are injected at generation time.
 */

import { GridConfig } from './types';

export function generatePrelude(grid: GridConfig): string {
  return `\
...
var COLS=${grid.cols}, ROWS=${grid.rows}, COUNT=COLS*ROWS;
...
function buildCtx(fb, st){
  ...
  return {
    ...
    uv:function(i){return [(i%COLS)/Math.max(COLS-1,1),((i/COLS)|0)/Math.max(ROWS-1,1)];},
